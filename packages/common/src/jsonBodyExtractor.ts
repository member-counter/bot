...
function jsonBodyExtractor(body: any, path: string): string {
  let cwo = body;
  let pathLeft = path;
  let pathTraveled = "";
  ...
  while (iterationsLeft) {
    ...
    const matchGroups = pathLeft.match(propExp)?.groups ?? {};
    ...
    if (matchGroups.propName) {
      ...
      if (!Object.prototype.hasOwnProperty.call(cwo, matched))
        throw new Error(
          `"${matched}" doesn't exists in ${pathTraveled ? pathTraveled : "the response"}`,
        );
