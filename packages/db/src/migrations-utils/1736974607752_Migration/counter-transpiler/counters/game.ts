...
const gameIds = {
  ...
  wolfensteinet: "wet",
  wurm: "wurmunlimited",
  // Extras that doesn't belong to gamedig but now is mapped to a gamedig gameid
  ragemp: "gta5r",
  ["minecraft-alt"]: "minecraft",
} as const;

function convertGameId(oldGameId: string | undefined): string | undefined {
  if (oldGameId == undefined) {
    return undefined;
  } else if (oldGameId in gameIds) {
    return gameIds[oldGameId as keyof typeof gameIds];
  } else {
    return oldGameId;
  }
}
