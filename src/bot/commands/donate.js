...
const cachedUserTags = new Map();

setInterval(() => {
    cachedUserTags.clear();
}, 24 * 60 * 60 * 1000);
                    ...
                    .then(async donators => {
                        ...
                        donators = await Promise.all(
                            donators.map(async donator => {
                                const userId = donator.user;
                                if (cachedUserTags.has(userId)) {
                                    donator.user = cachedUserTags.get(userId);
                                } else {
                                    await client.getRESTUser(userId)
                                        .then(restUser => {
                                            cachedUserTags.set(userId, restUser.username + "#" + restUser.discriminator);
                                            donator.user = cachedUserTags.get(userId);
                                        })
                                        .catch(() => {
