...
const newChannelNameCounter = {
  ...
  variants: [
    ...
    'createChannelNameCounter',
    'newCategoryNameCounter',
    'createCategoryNameCounter',
  ],
  allowedTypes: [0],
  requiresAdmin: true,
  run: ({ client, message, guildSettings, languagePack }) => {
    ...
    const channelType =
      args[0].toLowerCase() ===
        '{prefix}newcategorynamecounter'.replace(
          '{prefix}',
          guildSettings.prefix,
        ) ||
      args[0].toLowerCase() ===
        '{prefix}createcategorynamecounter'.replace(
          '{prefix}',
          guildSettings.prefix,
        )
        ? 4
    ...
    if (type && availableCounterTypes.includes(type.toLowerCase())) {
      ...
      if (type.toLowerCase() === 'memberswithrole') {
        ...
        if (channelName.length === 0) {
          ...
          roleMentions.forEach(
            (roleId) => (channelName += guild.roles.get(roleId).name + ', '),
          );
        ...
        let indexOfTypeInTheList = availableCounterTypes.findIndex(
          (item) => item === type.toLowerCase(),
        );
        channelName =
          channelName.length === 0
            ? `{COUNT} ${availableCounterTypesString[indexOfTypeInTheList]}`
            : channelName.join(' ');
      }
      ...
      if (
        type.toLowerCase() === 'bannedmembers' &&
        !userBotPerms.has('banMembers')
      ) {
        client
          .createMessage(
            channel.id,
            languagePack.commands.newChannelNameCounter.no_perms_ban,
          )
          .catch(console.error);
        return;
      }
      ...
      client
        ...
        .then((voiceChannel) => {
          guildSettings.channelNameCounters.set(voiceChannel.id, {
            channelName,
          ...
          guildSettings
            ...
            .then(() => {
              ...
              client
                .createMessage(
                  channel.id,
                  languagePack.commands.newChannelNameCounter.success,
                )
                .catch(console.error);
            })
            .catch((error) => {
              ...
              console.error(error);
              client
                .createMessage(channel.id, languagePack.common.error_unknown)
                .catch(console.error);
            });
        })
        .catch((e) => {
          ...
          if (e.code === 50013)
            client
              .createMessage(
                channel.id,
                languagePack.commands.newChannelNameCounter.no_perms,
              )
              .catch(console.error);
          else
            client
      ...
      client.createMessage(
        channel.id,
        languagePack.commands.newChannelNameCounter.error_invalid_params.replace(
          /\{PREFIX\}/gi,
          guildSettings.prefix,
        ),
      );
...
const topicCounter = {
  ...
  run: ({ client, message, guildSettings, languagePack }) => {
    ...
    if (content.toLowerCase().includes('all')) {
      channelsToPerformAction = guild.channels
    ...
    switch (action) {
        ...
        guildSettings
          ...
          .then(() => {
            ...
            channelsToPerformAction.forEach((channel, i) => {
              channelsToMention += `${i === 0 ? '' : ' '}<#${channel}>${
                i === channelsToPerformAction.length - 1 ? '.' : ','
              }`;
            });
            ...
            client
              .createMessage(
                channel.id,
                success_enable.replace('{CHANNELS}', channelsToMention),
              )
              .catch(console.error);
          })
          .catch((error) => {
        ...
        guildSettings
          ...
          .then(() => {
            ...
            channelsToPerformAction.forEach((channel_id, i) => {
              channelsToMention += `${i === 0 ? '' : ' '}<#${channel_id}>${
                i === channelsToPerformAction.length - 1 ? '.' : ','
              }`;
            });
            ...
            client
              .createMessage(
                channel.id,
                success_disable.replace('{CHANNELS}', channelsToMention),
              )
              .catch(console.error);
          })
          .catch((error) => {
      ...
      default:
        client
          .createMessage(
            channel.id,
            languagePack.commands.topicCounter.error_invalid_params.replace(
              /\{PREFIX\}/gi,
              guildSettings.prefix,
            ),
          )
    ...
    if (guildSettings.topicCounterChannels.size > 0) {
        ...
        if (i !== 0) {
          if (arg !== '') {
            if (arg.slice(0, 2) === '<#' && arg.slice(-1) === '>') {
              ...
              if (!channelsToCustomize.includes(channel_id))
                channelsToCustomize.push(channel_id);
            } else break;
          }
        }
      }
      ...
      if (channelsToCustomize.length > 0 && sliceFrom !== undefined) {
        ...
        guildSettings
          ...
          .then(() => {
            ...
            channelsToCustomize.forEach((channel, i) => {
              msgChannels += `${i === 0 ? '' : ' '}<#${channel}>${
                i === channelsToCustomize.length - 1 ? '.' : ','
              }`;
            });
            client
              .createMessage(
                channel.id,
                languagePack.commands.setTopic.success_unique.replace(
                  '{CHANNELS}',
                  msgChannels,
                ),
              )
        ...
        client
          .createMessage(
            channel.id,
            languagePack.commands.setTopic.no_topic.replace(
              /\{PREFIX\}/gi,
              guildSettings.prefix,
            ),
          )
        ...
        guildSettings
          ...
          .then(() => {
            updateCounter({ client, guildSettings });
            client
              .createMessage(channel.id, languagePack.commands.setTopic.success)
              .catch(console.error);
          })
          .catch((e) => {
            console.error(e);
      ...
      client
        .createMessage(
          channel.id,
          languagePack.common.no_topic_counter_enabled.replace(
            /\{PREFIX\}/gi,
            guildSettings.prefix,
          ),
        )
        ...
        guildSettings
          ...
          .then(() => {
            ...
            mentionedChannels.forEach((channel, i) => {
              stringMentionedChannels += `${i === 0 ? '' : ' '}<#${channel}>${
                i === mentionedChannels.length - 1 ? '.' : ','
              }`;
            });
            ...
            client
              .createMessage(
                channel.id,
                languagePack.commands.removeTopic.success_unique.replace(
                  '{CHANNELS}',
                  stringMentionedChannels,
                ),
              )
              .catch(console.error);
          })
        ...
        guildSettings
          ...
          .then(() => {
            ...
            client
              .createMessage(
                channel.id,
                languagePack.commands.removeTopic.success,
              )
              .catch(console.error);
          })
          .catch((e) => {
...
const update = {
  ...
  run: async ({ client, message, guildSettings, languagePack }) => {
    const { channel } = message;
    client
      .createMessage(channel.id, languagePack.commands.update.inProgress)
      .catch(console.error);
    const updateCounterStatus = await updateCounter({ client, guildSettings });
    if (!updateCounterStatus) {
      client
        .createMessage(channel.id, languagePack.commands.update.success)
        .catch(console.error);
    } else {
      client
        .createMessage(channel.id, languagePack.commands.update.wait)
        .catch(console.error);
    }
  },
};
