...
function checkConfig() {
  ...
  if (
    ...
    isNaN(Number(Buffer.from(DISCORD_CLIENT_TOKEN.split(".")[0], "base64").toString()))
  ) {
  ...
  if (!UNRESTRICTED_MODE) console.warn("[ENV WARN] UNRESTRICTED_MODE is set to false, some counters won't be available");
  if (!PREMIUM_BOT) console.warn("[ENV WARN] PREMIUM_BOT is set to false, some counters won't be available");
  if (GHOST_MODE) console.warn("[ENV WARN] GHOST_MODE is set to true, counters won't be updated and commands won't work!");
  ...
  if (!availableLanguagePacks.includes(DISCORD_DEFAULT_LANG)) {
    console.error(`[ENV ERROR] The current language code "${DISCORD_DEFAULT_LANG}" is invalid`);
    fail = true;
  }
  ...
  if (twitchCredentialsCheckNull.length && twitchCredentialsCheck.length !== twitchCredentialsCheckNull.length) {
    console.error(`[ENV ERROR] WITCH_CLIENT_ID or TWITCH_CLIENT_SECRET is/are empty, empty the vars or fill out all`);
    fail = true;
  }

  const twitterCredentialsCheck = [TWITTER_CONSUMER_KEY, TWITTER_CONSUMER_SECRET, TWITTER_ACCESS_TOKEN, TWITTER_ACCESS_TOKEN_SECRET];
  const twitterCredentialsCheckNull = twitterCredentialsCheck.filter(x => x);
  if (twitterCredentialsCheckNull.length && twitterCredentialsCheck.length !== twitterCredentialsCheckNull.length) {
    console.error(`[ENV ERROR] TWITTER_CONSUMER_KEY or TWITTER_CONSUMER_SECRET or TWITTER_ACCESS_TOKEN or TWITTER_ACCESS_TOKEN_SECRET is/are empty, empty the vars or fill out all`);
    fail = true;
  }
